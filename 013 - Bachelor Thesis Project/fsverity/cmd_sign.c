		      ...
		      int argc, char *argv[])
{
    ...
    // Allocating memory
    digest_bin = xzalloc(sizeof(*digest_bin) * 32);
    digest = xzalloc(sizeof(*digest) * 64);
    digest_hexa = xzalloc(sizeof(*digest_hexa) * 64);
    
	// Converting to binary
    strncpy(digest_hexa, argv[0], SHA256_DIGEST_SIZE*2);
    hex2bin((const char *)digest_hexa, digest_bin, strlen(digest_hexa) / 2);
    
	// Initialising Digest
    digest->digest_algorithm = FS_VERITY_HASH_ALG_SHA256;
    digest->digest_size = SHA256_DIGEST_SIZE;
    memcpy(digest->digest, digest_bin, digest->digest_size);
    ...
    // Test output
    // printf("Digest bin: %s\nDigest: %s\n", digest_bin, digest->digest);
    // printf("\n\ndigest_hex: %s\nLength digest_hex: %ld\n", digest_hex, strlen(digest_hex));
    // printf("\n\nDigest size: %d\nLength bin: %ld\nDigest: %s\n", digest->digest_size, strlen(digest->digest), digest->digest);
	...
	//        libfsverity_get_hash_name(digest->digest_algorithm), digest_hex, digest->digest);
    printf("Signature file successfully generated.");
	status = 0;
out:
	//filedes_close(&file);
